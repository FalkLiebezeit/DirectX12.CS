...
{
    public class wiredFountainApp : D3DApp
    {
        ...
        private readonly Dictionary<string, MeshGeometry> _geometries = new();


        private readonly Dictionary<string, ShaderBytecode> _shaders = new();
        private readonly Dictionary<string, PipelineState> _psos = new();
        ...
        private Vector3 _peakPos = new Vector3(0.0f,0.0f, 0.0f);
        ...
        private float _phi = MathUtil.PiOverTwo - 0.43f;
        private float _radius = 60.0f;

        private float _tBase;

        private Point _lastMousePos;

        public wiredFountainApp()
        {
            MainWindowCaption = "wired Fountain";
        }
        ...
        public override void Initialize()
        {
            ...
            _peakPos = new Vector3(_waves.RowCount/2,0.0f,_waves.ColumnCount/2);
        ...
        private void UpdateCamera(GameTimer gt)
        {
            ...
            _eyePos.X = _radius * MathHelper.Sinf(_phi) * MathHelper.Cosf( 0 * _theta * gt.TotalTime / 100.0f);
            _eyePos.Z = _radius * MathHelper.Sinf(_phi) * MathHelper.Sinf(0 * _theta * gt.TotalTime / 100.0f);
        ...
        private void UpdateWaves(GameTimer gt)
        {
            ...
            if ((Timer.TotalTime - _tBase) >= 0.25f)
            {
                ...
                int i = MathHelper.Rand(0, 2);
                int j = MathHelper.Rand(0, 2);
               

                _peakPos.X += dx;

                if (_peakPos.X < 10 || _peakPos.X > 118) dx *= -1;
                       

                _peakPos.Z += 0.0f * dz;
                

                float height = MathHelper.Randf(0.2f, 1.2f);

                _waves.Disturb((int)_peakPos.X,(int) _peakPos.Z, height);
 */
               ...
               */
                ...
                float r = 0.80f; // MathHelper.Randf(0.2f, 1.0f);

                _waves.Disturb((int)_peakPos.X, (int)_peakPos.Z, r);

            }

            // Update the wave simulation.
            _waves.Update(gt.DeltaTime * 1);
        ...
        private static float GetHillsHeight(float x, float z) => 0.5f * (z * MathHelper.Sinf(0.1f * x) + x * MathHelper.Cosf(0.1f * z));
    }
}
