...
namespace DX12GameProgramming
{
    internal class Grid
    {
        ...
        public Grid(int m,  float dx, float dt)
        {
            float speed = 4.0f;
            float damping = 0.2f;
            ...
            for (int i = 0; i < m; i++)
            {
                ...
                for (int j = 0; j < m; j++)
                {
                    ...
                    float y = GetFuncValue(x, z);

                    _prevSolution[i * m + j] = new Vector3(x, y, z);
                    _currSolution[i * m + j] = new Vector3(x, y, z);

                    _normals[i * m + j] = Vector3.UnitY;
                    _tangentX[i * m + j] = Vector3.UnitX;
                }
            }
        }
        ...
        public float Height => RowCount * _spatialStep;
        ...
        public void Disturb(int i, float magnitude)
        {
            // Don't disturb boundaries.
            Debug.Assert(i > 1 && i < RowCount - 2);
            Debug.Assert(i > 1 && i < ColumnCount - 2);

            float halfMag = 0.5f * magnitude;

            // Disturb the iith vertex height and its neighbors.
            _currSolution[i * ColumnCount + i].Y += magnitude;

            _currSolution[i * ColumnCount + i + 1].Y += halfMag;
            _currSolution[i * ColumnCount + i - 1].Y += halfMag;
            _currSolution[(i + 1) * ColumnCount + i].Y += halfMag;
            _currSolution[(i - 1) * ColumnCount + i].Y += halfMag;
        }

        private static float GetFuncValue0(float x, float z) => 10.0f * (float)Math.Exp(-(Math.Pow((x / 2.0f), 2) + Math.Pow((z / 2.0f), 2)));
        ...
        private static float GetFuncValue(float x, float z)
        {
            ...
            double result = (10.0f * (Math.Sin(r)) / (r));
