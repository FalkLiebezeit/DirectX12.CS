...
{
    public class SpheresJumpingCamera : D3DApp
    {
        ...
        private RenderItem _sphereRitem00;
        private RenderItem _sphereRitem01;
        ...
        */
        ...
        private float _theta    = 1.75f * MathUtil.Pi;
        private float _phi      = 0.425f * MathUtil.Pi;
        ...
        public SpheresJumpingCamera()
        {
            MainWindowCaption = "simple jumping spheres with camera";
        }
        ...
        public override void Initialize()
        {
            ...
            _camera.Position = new Vector3(0.0f, 2.20f, -18.0f); // Camera Position
        ...
        protected override void Update(GameTimer gt)
        {
            ...
            //
            // Animate the shapes the center sphere.
            //
            ...
            float vel = 2.5f;
            float gloVel = 0.50f;
            float y = 2 * Math.Abs( MathHelper.Sinf(_phi * gt.TotalTime)) + 0.5f;

            Matrix sphereScale = Matrix.Scaling(1.0f);

            Matrix sphereOffset = Matrix.Translation(0.0f, y, 7.0f);

            

            Matrix sphereLocalRotate = Matrix.RotationX(vel * gt.TotalTime) *
                                            Matrix.RotationY(vel * gt.TotalTime) *
                                                Matrix.RotationZ(vel * gt.TotalTime);

            Matrix sphereGlobalRotate = Matrix.Identity;
            sphereGlobalRotate *= Matrix.RotationY(gloVel * gt.TotalTime);
           


            _sphereRitem00.World = sphereScale * sphereLocalRotate * sphereOffset * sphereGlobalRotate;
            _sphereRitem00.NumFramesDirty = NumFrameResources;
            ...
            vel = 2.5f;
            gloVel = 0.30f;
            y = 4 * Math.Abs(MathHelper.Sinf(_phi * gt.TotalTime)) + 0.5f;

            sphereScale = Matrix.Scaling(1.0f);
            sphereOffset = Matrix.Translation(0.0f, y, 7.0f);

            sphereLocalRotate = Matrix.RotationX(vel * gt.TotalTime) *
                                            Matrix.RotationY(vel * gt.TotalTime) *
                                                Matrix.RotationZ(vel * gt.TotalTime);

            sphereGlobalRotate = Matrix.Identity;
            sphereGlobalRotate *= Matrix.RotationY(gloVel * gt.TotalTime);



            _sphereRitem01.World = sphereScale * sphereLocalRotate * sphereOffset * sphereGlobalRotate;
            _sphereRitem01.NumFramesDirty = NumFrameResources;
        ...
        protected override void Draw(GameTimer gt)
        {
            ...
            CommandList.Reset(cmdListAlloc, _isWireframe ? _psos["opaque_wireframe"] : _psos["opaque"]);   // --> wireframe
        ...
        */
        ...
        private void BuildShapeGeometry()
        {
            ...
            SubmeshGeometry sphere00 = AppendMeshData(GeometryGenerator.CreateSphere(0.5f, 20, 20), Color.Crimson, vertices, indices);
            SubmeshGeometry sphere01 = AppendMeshData(GeometryGenerator.CreateSphere(0.5f, 20, 20), Color.DarkSlateBlue, vertices, indices);
            ...
            geo.DrawArgs["sphere00"] = sphere00;
            geo.DrawArgs["sphere01"] = sphere01;
        ...
        private void BuildRenderItems()
        {
            ...
            _sphereRitem00 = AddRenderItem(RenderLayer.Opaque, j++, "shapeGeo", "sphere00",
                   world: Matrix.Translation(0.0f, 0.0f, 0.0f));

            
            _sphereRitem01 = AddRenderItem(RenderLayer.Opaque, j++, "shapeGeo", "sphere01",
                   world: Matrix.Translation(0.0f, 0.0f, 0.0f));
