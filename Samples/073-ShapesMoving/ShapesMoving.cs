...
{
    public class ShapesMoving : D3DApp
    {
        ...
        private RenderItem _boxRitem;
        private RenderItem _sphereRitem;
        private RenderItem _cylinderRitem;
        ...
        public ShapesMoving()
        {
            MainWindowCaption = "moving objects";
        }
        ...
        protected override void Update(GameTimer gt)
        {
            ...
            Matrix boxScale = Matrix.Scaling(2.0f);

            Matrix boxOffset = Matrix.Translation(0.0f,1.0f, 0.0f);
            Matrix boxLocalRotate = Matrix.RotationY(-0.1f * gt.TotalTime);
            Matrix boxGlobalRotate =  Matrix.Identity;

            _boxRitem.World = boxScale * boxOffset * boxLocalRotate  * boxGlobalRotate;
            _boxRitem.NumFramesDirty = NumFrameResources;
            ...
            Matrix sphereScale = Matrix.Scaling(2.0f);
            Matrix sphereOffset = Matrix.Translation(0.0f, 1.0f, 7.0f);
            ...
            sphereGlobalRotate *= Matrix.RotationY(0.5f * gt.TotalTime);
           


            _sphereRitem.World = sphereScale  * sphereLocalRotate * sphereOffset  * sphereGlobalRotate;
            _sphereRitem.NumFramesDirty = NumFrameResources;


            // cylinder

            Matrix cylinderScale = Matrix.Scaling(1.0f);
            Matrix cylinderOffset = Matrix.Translation(0.0f, 1.50f, 5.0f);

            Matrix cylinderLocalRotate = Matrix.Identity;
            cylinderLocalRotate *= Matrix.RotationY(0.5f * gt.TotalTime);
            
            Matrix cylinderGlobalRotate = Matrix.Identity;


            _cylinderRitem.World = cylinderScale * cylinderLocalRotate * cylinderOffset * cylinderGlobalRotate;
            _cylinderRitem.NumFramesDirty = NumFrameResources;
        ...
        private void BuildShapeGeometry()
        {
            ...
            SubmeshGeometry cylinder = AppendMeshData(GeometryGenerator.CreateCylinder(0.5f, 0.3f, 3.0f, 20, 20), Color.Blue, vertices, indices);
        ...
        private void BuildRenderItems()
        {
            ...
            _boxRitem = AddRenderItem(RenderLayer.Opaque,j++, "shapeGeo", "box",
                world: Matrix.Scaling(2.0f, 2.0f, 2.0f) * Matrix.Translation(0.0f, 2.0f, -5.0f));

            _cylinderRitem = AddRenderItem(RenderLayer.Opaque,j++, "shapeGeo", "cylinder",
                    world: Matrix.Translation(-2.0f, 2.5f, 10.0f ));

            _sphereRitem = AddRenderItem(RenderLayer.Opaque, j++, "shapeGeo", "sphere",
