...
{
    public class movPeakApp : D3DApp
    {
        ...
        private float _phi = MathUtil.PiOverTwo - 0.45f;
        ...
        private void UpdateWaves(GameTimer gt)
        {
            ...
            if ((Timer.TotalTime - _tBase) >= 0.25f)
            {
                ...
                if (_peakPos.X < 4 || _peakPos.X >= _waves.RowCount-4) dx *= -1;



                _peakPos.Z += dz;

                if (_peakPos.Z < 4 || _peakPos.Z >= _waves.RowCount-4) dz *= -1;


               


                float height = MathHelper.Randf(0.2f, 3.0f);

                //_waves.Disturb(64,64, height);
