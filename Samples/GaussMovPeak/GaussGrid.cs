...
namespace DX12GameProgramming
{
    internal class Grid
    {
        ...
        public void Update_old(float dt)
        {
            // Accumulate time.
            _t +=  dt;
        ...
        public void Update(float dt, int _peakPosX)
        {
            // Accumulate time.
            _t += dt / 2f ;
            ...
            if (_t >= _timeStep)
            {
                ...
                //for(int i = 1; i < RowCount - 1; i++)
                {
                    for (int j = 1; j < ColumnCount - 1; j++)
                    {
                       ...
                       // if (_currSolution[i * ColumnCount + j].X > 11) _peakPosX = 0;

                        _currSolution[i * ColumnCount + j].Y =

                           _prevSolution[(i + -1 * _peakPosX) * ColumnCount + j].Y ;

                           /*

                            0.0f * _currSolution[i * ColumnCount + j].Y +

                            0.0f * (_currSolution[(i + 1) * ColumnCount + j].Y +
                                    _currSolution[(i - 1) * ColumnCount + j].Y +
                                    _currSolution[i * ColumnCount + j + 1].Y +
                                    _currSolution[i * ColumnCount + j - 1].Y);
                           */
        ...
        private static float GetFuncValue(float x, float z) => 8.0f * (float)Math.Exp(-(Math.Pow((x / 2.50f), 2) + Math.Pow((z / 2.50f), 2)));
