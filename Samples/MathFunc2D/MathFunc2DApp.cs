...
{
    public class MathFunc2DApp : D3DApp
    {
        ...
        private RenderItem _graphRitem;
        ...
        private float _phi = MathUtil.PiOverTwo - 0.3f;
        ...
        public MathFunc2DApp()
        {
            MainWindowCaption = "basic 2D Mathematical Function";
        }
        ...
        public override void Initialize()
        {
            ...
            //public Wave(int m, int n, float dx, float dt, float speed, float damping)  -> Wave.cs
            _graph = new Graph(64, 8, 0.5f, 0.02f, 4.0f, 0.2f); // see line 311
        ...
        //0.3f * (z * MathHelper.Sinf(0.01f * x) + x * MathHelper.Cosf(0.01f * z));
        //private static float GetHillsHeight(float x, float z) => 0.3f * (z * MathHelper.Sinf(0.5f * x) + x * MathHelper.Cosf(0.5f * z));
        //private static float GetHillsHeight(float x, float z) => 1.3f * MathHelper.Sinf(1.5f * x * x) + MathHelper.Cosf(1.5f * z * z);
        //private static float GetGraph(float x,float z) => 1.3f * MathHelper.Sinf(1.5f * x * x);
        ...
        private void UpdateWaves(GameTimer gt)
        {
            ...
            if (_once) 
            {
                ...
                float height = 4.0f; // see Graph.cs line 38      MathHelper.Randf(0.0f, 4.0f);

                _graph.Disturb(32, 4, height);  // see line 71
            }

            // Update the wave simulation.
            _graph.Update(gt.DeltaTime * 0.1f);
            ...
            _graphRitem.Geo.VertexBufferGPU = currWavesVB.Resource;
        }
        ...
        private void BuildLandGeometry()
        {
            ...
            for (int i = 0; i < grid.Vertices.Count; i++)
            {
                ...
                //vertices[i].Pos.Y = GetGraph(p.X, p.Z);
                //vertices[i].Pos.Y = GetGraph(p.X);
        ...
        {
            var indices = new short[3 * _graph.TriangleCount]; // 3 indices per face.

            Debug.Assert(_graph.VertexCount < 0x0000ffff);

            // Iterate over each quad.
            int m = _graph.RowCount;
            int n = _graph.ColumnCount;
            int k = 0;
            ...
            geo.VertexBufferByteSize = geo.VertexByteStride * _graph.VertexCount;
        ...
        private void BuildFrameResources()
        {
            ...
            {
                _frameResources.Add(new FrameResource(Device, 1, _allRitems.Count, _graph.VertexCount));
        ...
        {
            _graphRitem = AddRenderItem(RenderLayer.Opaque, 0, "waterGeo", "grid");
