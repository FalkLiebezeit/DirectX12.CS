...
namespace DX12GameProgramming
{
    internal class Graph
    {
        ...
        public Graph(int m, int n, float dx, float dt, float speed, float damping)
        {
            RowCount = m;
            ColumnCount = n;
        ...
        public void Disturb(int i, int j, float magnitude)
        {
            ...
            */
            ...
            // Disturb the ijth vertex height and its neighbors.
            _currSolution[i * ColumnCount + j + 0].Y += magnitude;

            _currSolution[i * ColumnCount + j + 1].Y += magnitude;
            _currSolution[i * ColumnCount + j + 2].Y += magnitude;
            _currSolution[i * ColumnCount + j + 3].Y += magnitude;
           // _currSolution[i * ColumnCount + j + 4].Y += magnitude;
            ...
            _currSolution[(i + 0) * ColumnCount + j - 1].Y +=halfMag ;

            _currSolution[(i + 1) * ColumnCount + j].Y += halfMag;
            _currSolution[(i - 1) * ColumnCount + j].Y += halfMag;

            _currSolution[(i + 2) * ColumnCount + j].Y += halfMag;
            _currSolution[(i - 2) * ColumnCount + j].Y += halfMag;

            _currSolution[(i + 3) * ColumnCount + j].Y += halfMag;
            _currSolution[(i - 3) * ColumnCount + j].Y += halfMag;
            */
        }
    }
